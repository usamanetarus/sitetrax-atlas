...
**Prereqs:** Python 3.11+, the `gcloud` CLI, and a GCP project with billing and the
**Vertex AI API** enabled (your account needs *Vertex AI User* or broader).
...
Everything is configured through `.env` (see `.env.example`). The defaults give you a
fully working local demo; each block below unlocks one capability.
...
`deploy.sh` builds and pushes the image, deploys to **Cloud Run** with the env vars from
your `.env`, optionally deploys the remote MCP server, and wires up **Cloud Scheduler**:
